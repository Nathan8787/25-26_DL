"""
Kaggle-friendly version of HW3 script.
Reads data from DATA_DIR (default: /kaggle/input/hw3data), writes artifacts to BASE_DIR (default: /kaggle/working).
Uses GPU if available; follows update_spec requirements.
"""
import os
import json
import csv
...
def default_paths():
    base_dir = os.getenv("BASE_DIR", "/kaggle/working")
    data_dir = os.getenv("DATA_DIR", "/kaggle/input/hw3data")
    return base_dir, data_dir


@dataclass
class Config:
    base_dir, data_dir = default_paths()
    train_file: str = os.path.join(data_dir, "shakespeare_train.txt")
    valid_file: str = os.path.join(data_dir, "shakespeare_valid.txt")
    batch_size: int = 256
    seq_len: int = 100
    model_type: str = "RNN"
    hidden_size: int = 128
...
def build_experiments(base_cfg: Config) -> List[Dict]:
    exps: List[Dict] = []
    # Baseline RNN
    exps.append({"name": "RNN", "model_type": "RNN", "hidden_size": 128, "seq_len": 100})
    # RNN hidden size comparison
    for hs in [64, 256]:
        exps.append({"name": f"RNN_HS_{hs}", "model_type": "RNN", "hidden_size": hs, "seq_len": 100})
    # RNN sequence length comparison
    for sl in [50, 150]:
        exps.append({"name": f"RNN_SL_{sl}", "model_type": "RNN", "hidden_size": 128, "seq_len": sl})
    # Baseline LSTM
    exps.append({"name": "LSTM", "model_type": "LSTM", "hidden_size": 128, "seq_len": 100})
    # LSTM hidden size comparison
    for hs in [64, 256]:
        exps.append({"name": f"LSTM_HS_{hs}", "model_type": "LSTM", "hidden_size": hs, "seq_len": 100})
    # LSTM sequence length comparison
    for sl in [50, 150]:
        exps.append({"name": f"LSTM_SL_{sl}", "model_type": "LSTM", "hidden_size": 128, "seq_len": sl})
...
def write_summary_rnn_vs_lstm(outputs_dir: str, metrics_map: Dict[str, List[Dict]], plot_refs: List[str]) -> None:
...
def main():
    ...
    if not os.path.exists(cfg.train_file) or not os.path.exists(cfg.valid_file):
        ...
        print(f"Expected: {cfg.train_file}, {cfg.valid_file}")
        return
    ...
    for exp in experiments:
        ...
        train_loader = DataLoader(
            train_ds, batch_size=exp_cfg.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True
        )
        valid_loader = DataLoader(
            valid_ds, batch_size=exp_cfg.batch_size, shuffle=False, num_workers=4, pin_memory=True, drop_last=True
        )
    ...
    plot_curves(
        loss_plots_hidden_rnn,
        "Loss",
        "RNN Hidden Size Comparison (Loss)",
        os.path.join(cfg.plots_dir, "comparison_hidden_size.png"),
    )
    plot_curves(
        err_plots_hidden_rnn,
        "Error Rate",
        "RNN Hidden Size Comparison (Error)",
        os.path.join(cfg.plots_dir, "comparison_hidden_size_error.png"),
    )
    plot_curves(
        bpc_plots_hidden_rnn,
        "BPC",
        "RNN Hidden Size Comparison (BPC)",
        os.path.join(cfg.plots_dir, "comparison_hidden_size_bpc.png"),
    )

    plot_curves(
        loss_plots_seq_rnn,
        "Loss",
        "RNN Sequence Length Comparison (Loss)",
        os.path.join(cfg.plots_dir, "comparison_seq_len.png"),
    )
    plot_curves(
        err_plots_seq_rnn,
        "Error Rate",
        "RNN Sequence Length Comparison (Error)",
        os.path.join(cfg.plots_dir, "comparison_seq_len_error.png"),
    )
    plot_curves(
        bpc_plots_seq_rnn,
        "BPC",
        "RNN Sequence Length Comparison (BPC)",
        os.path.join(cfg.plots_dir, "comparison_seq_len_bpc.png"),
    )

    plot_curves(
        loss_plots_hidden_lstm,
        "Loss",
        "LSTM Hidden Size Comparison (Loss)",
        os.path.join(cfg.plots_dir, "comparison_lstm_hidden_size.png"),
    )
    plot_curves(
        err_plots_hidden_lstm,
        "Error Rate",
        "LSTM Hidden Size Comparison (Error)",
        os.path.join(cfg.plots_dir, "comparison_lstm_hidden_size_error.png"),
    )
    plot_curves(
        bpc_plots_hidden_lstm,
        "BPC",
        "LSTM Hidden Size Comparison (BPC)",
        os.path.join(cfg.plots_dir, "comparison_lstm_hidden_size_bpc.png"),
    )

    plot_curves(
        loss_plots_seq_lstm,
        "Loss",
        "LSTM Sequence Length Comparison (Loss)",
        os.path.join(cfg.plots_dir, "comparison_lstm_seq_len.png"),
    )
    plot_curves(
        err_plots_seq_lstm,
        "Error Rate",
        "LSTM Sequence Length Comparison (Error)",
        os.path.join(cfg.plots_dir, "comparison_lstm_seq_len_error.png"),
    )
    plot_curves(
        bpc_plots_seq_lstm,
        "BPC",
        "LSTM Sequence Length Comparison (BPC)",
        os.path.join(cfg.plots_dir, "comparison_lstm_seq_len_bpc.png"),
    )
    ...
    plot_curves(
        ...
        "Loss",
        "RNN vs LSTM (Loss)",
        os.path.join(cfg.plots_dir, "comparison_rnn_vs_lstm.png"),
    )
    plot_curves(
        err_plots_rnn_vs_lstm,
        "Error Rate",
        "RNN vs LSTM (Error)",
        os.path.join(cfg.plots_dir, "comparison_rnn_vs_lstm_error.png"),
    )
    plot_curves(
        bpc_plots_rnn_vs_lstm,
        "BPC",
        "RNN vs LSTM (BPC)",
        os.path.join(cfg.plots_dir, "comparison_rnn_vs_lstm_bpc.png"),
    )
